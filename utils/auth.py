...
from core.settings import JWT_EXPIRED, SECRET_KEY, JWT_MODEL
...
def _get_user_model():
    ...
    :return: [db.Model] 用户类
    """
    user_model = importlib.import_module(JWT_MODEL)
    return user_model
...
def general_token(user):
    ...
    if hasattr(user, user_model.USERNAME_FILED):
        username = getattr(user, user_model.USERNAME_FILED)
        payload = {
            "exp": expire_time,
            "id": user.id,
            "username": username
